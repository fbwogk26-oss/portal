...
export async function registerRoutes(
  ...
  // === ACCESS REQUEST WITH EMAIL ===
  app.post('/api/access/submit-with-email', async (req, res) => {
    try {
      const { title, content, category } = req.body;
      
      // 1. Create the notice first
      const notice = await storage.createNotice({ title, content, category });
      ...
      const buffer = await workbook.xlsx.writeBuffer();
      const today = new Date().toISOString().slice(0, 10).replace(/-/g, '.');
      const filename = `kt MOS남부 대구본부 ${visitPurpose} 을를 위한 출입신청(출입장소 ${entranceLocation})_${today}.xlsx`;

      // 3. Send email with Resend
      const resendApiKey = process.env.RESEND_API_KEY;
      if (!resendApiKey) {
        return res.status(500).json({ message: "Email configuration missing", noticeId: notice.id });
      }
      ...
      const emailResult = await resend.emails.send({
        ...
        html: `
          ...
          <p><strong>방문목적:</strong> ${visitPurpose}</p>
          <p><strong>출입장소:</strong> ${entranceLocation}</p>
          <p><strong>방문기간:</strong> ${formattedStart} ${startTime} ~ ${formattedEnd || formattedStart} ${endTime}</p>
          <p><strong>인솔자:</strong> ${supervisorDept} / ${supervisorName} (${supervisorPhone})</p>
          <p><strong>신청인원:</strong> ${people.length}명</p>
          <br/>
          <p>첨부된 엑셀 파일을 확인해주세요.</p>
        `,
        attachments: [
          {
            filename: filename,
      ...
      console.error('Access submit with email error:', err);
