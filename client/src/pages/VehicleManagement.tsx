...
import { Car, Plus, Trash2, FileText, ImagePlus, X, Upload, Download, FileSpreadsheet } from "lucide-react";
...
export default function VehicleManagement() {
  const { data: materials, isLoading } = useNotices("vehicle");
  ...
  const handleAdd = () => {
    ...
    createMaterial({ title, content: contentData, category: "vehicle", imageUrl: imageUrl || undefined }, {
      onSuccess: () => {
        setTitle("");
        setContent("");
        setImageUrl(null);
        setExcelUrl(null);
        setExcelName(null);
        toast({ title: "자료 추가 완료", description: "차량관리 자료가 게시되었습니다." });
      }
    });
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-display font-bold text-foreground flex items-center gap-3">
            <div className="bg-cyan-100 p-2 rounded-xl text-cyan-600 dark:bg-cyan-900/30 dark:text-cyan-400">
              <Car className="w-8 h-8" />
            </div>
            차량 관리
          </h2>
          <p className="text-muted-foreground mt-2">차량 관리 자료 및 문서를 관리합니다.</p>
        </div>
      </div>

      <Card className="glass-card overflow-hidden border-cyan-200 dark:border-cyan-900/30">
        <CardContent className="p-6 space-y-4">
          ...
          <Textarea 
            ...
            disabled={isLocked}
            data-testid="input-vehicle-content"
          />
          ...
          <input
            ...
            className="hidden"
            data-testid="input-vehicle-excel"
          />
          ...
          <div className="flex flex-wrap gap-2">
            {imageUrl ? (
              ...
              <Button
                ...
                className="gap-2"
                data-testid="button-add-vehicle-image"
              >
            ...
            {excelUrl ? (
              <div className="flex items-center gap-2 px-3 py-2 bg-green-50 border border-green-200 rounded-lg dark:bg-green-900/20 dark:border-green-800">
                ...
                <Button
                  ...
                  data-testid="button-remove-vehicle-excel"
                >
                  <X className="w-3 h-3" />
                </Button>
              </div>
              ...
              <Button
                ...
                className="gap-2"
                data-testid="button-add-vehicle-excel"
              >
          ...
          <div className="flex justify-end">
            <Button onClick={handleAdd} disabled={isLocked || isCreating || !title} className="bg-cyan-600 hover:bg-cyan-700 text-white gap-2" data-testid="button-add-vehicle">
          ...
          {materials?.map((item) => {
            ...
            return (
              <motion.div
                ...
                data-testid={`card-vehicle-${item.id}`}
              >
                <div className="space-y-4">
                  <div className="flex justify-between items-start">
                    <div className="p-3 bg-cyan-50 text-cyan-600 rounded-lg dark:bg-cyan-900/20">
                    ...
                    <Button 
                      ...
                      disabled={isLocked}
                      data-testid={`button-delete-vehicle-${item.id}`}
                    >
